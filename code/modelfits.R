...
dt<-dt[,c("yearmon","cdom","chlaiv","phycoe","c6chl","c6cdom","phycoc","intercept","rsquared","pvalue")]
dt[,2:8]<-round(dt[,2:8],4)
dt[,"rsquared"]<-round(dt[,"rsquared"],2)
dt[,"pvalue"]<-round(dt[,"pvalue"],2)
dt$pvalue[dt$pvalue==0]<-"< 0.00"

write.csv(dt,"/home/jose/Documents/Science/JournalSubmissions/DataflowChl/tables/modelfits.csv",row.names = FALSE)