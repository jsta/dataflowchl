grabs <- read.csv(file.path("data", "allgrabs_log.csv"), 
                  row.names = NULL, stringsAsFactors = FALSE)

grabs$location <- factor(grabs$location, levels = c(
  "Whipray Basin", "Rankin", "Terrapin Bay", "Monroe Lake", 
  "Seven Palm Lake", "Madeira Bay", "Little Madeira", "Pond 5", "Joe Bay", 
  "Long Sound", "Blackwater Sound", "Lake Surprise", "Manatee", "Barnes"))
...
p.values <- apply(comb_names, 1, 
                  function(x) round(
                    cor.test(grabs_cor[,x[1]], 
                             grabs_cor[,x[2]], 
...
grabs_cor <- matrix(round(cor(grabs_cor, method = "spearman"), 2), 
                    ncol = ncol(grabs_cor))

grabs_cor[lower.tri(grabs_cor)] <- NA

grabs_cor[seq(from = 1, to = length(grabs_cor), 
              by = dim(grabs_cor)[1] + 1)] <- NA

p.values[lower.tri(p.values)] <- NA
p.values[seq(from = 1, to = length(p.values), 
             by = dim(p.values)[1] + 1)] <- NA
...
names(grabs_cor) <- names(grabs[,data_fields])[
  2:length(names(grabs[,data_fields]))]
