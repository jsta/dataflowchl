...
dtsub <- dt[dt$collection.date >= "2008-01-01" & 
              dt$collection.date <= "2015-09-30",]


testfull <- suppressWarnings(
  aggregate(dtfull, by = list(
    Dates = dtfull$collection.date, 
    Zone = dtfull$zone), mean)[
      ,c("chl.a.ug.l", "collection.date", "Zone")])
testsub <- suppressWarnings(
  aggregate(dtsub, by = list(
    Dates = dtsub$collection.date, 
    Zone = dtsub$zone), mean)[
      ,c("chl.a.ug.l", "collection.date", "Zone")])

testfull <- testfull[!(testfull$Zone %in% c("FBS", "CEB", "FBW")),]
testsub <- testsub[!(testsub$Zone %in% c("FBS", "CEB", "FBW")),]

rain <- read.csv("data/rain/ENPOps_rain_monthly.csv", 
                 stringsAsFactors = FALSE)
rain$date <- as.POSIXct(rain$date)
rain <- rain[rain$date >= range(testsub$collection.date)[1] & 
               rain$date <= range(testsub$collection.date)[2],]
rain[,!(names(rain) %in% "date")] <- rain[,!(names(rain) %in% "date")] * 2.54 
# convert in to cm
...
## Restricted POR chl =====================================#
plotsub <- ggplot(data = testsub, aes(
  x = collection.date, 
  y = chl.a.ug.l, 
  colour=Zone, 
  linetype = Zone))
...
plotsub <- plotsub + scale_linetype_manual(values=c(1,2,2)) + 
  scale_color_manual(values=viridis(3))

## Rain ===================================================#
plot_rain <- ggplot(data = rain, aes(x = date, y = TaylorSlough))
plot_rain <- plot_rain + geom_line(na.rm = TRUE) + 
  geom_point(na.rm = TRUE) + xlab("") + 
  theme(
    axis.title = element_text(size = 10, face = "bold")) + 
  ylab("Precipitation(cm)")

## Figure montage + save ####################################
png(filename = "figures/chltimeseries.png", 
    width=1120, height =1400, res = 300)

plot_grid(plotfull, plotsub, plot_rain, align = "v", ncol = 1, 
          labels = "auto", hjust = -6, vjust = 1.7)
...
testfull_cb$period <- case_when(
  as.Date(testfull_cb$collection.date) < as.Date("2008-01-01") & 
    as.Date(testfull_cb$collection.date) > as.Date("2001-12-30") ~ "early",
  as.Date(testfull_cb$collection.date) > as.Date("2008-01-01") ~ "late")
testfull_cb <- group_by(testfull_cb, period)
(testfull_cb_stats <- summarise(testfull_cb, 
                                mean = mean(chl.a.ug.l, na.rm = TRUE), 
                                se = 2 * sd(chl.a.ug.l, na.rm = TRUE) / 
                                  length(chl.a.ug.l)))
